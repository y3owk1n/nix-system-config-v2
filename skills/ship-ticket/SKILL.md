...
4. Run `cursor-team-kit/deslop` to clean AI-specific slop patterns — unnecessary comments, defensive try/catch, `any` casts, deep nesting. Keep behavior unchanged.
5. **Regenerate whatever the change invalidated** — every checked-in generated artifact the repo compares byte for byte. That is the one failure that is certain rather than probable, and the one CI cannot tell you anything you did not already know.
6. `git fetch origin && git rebase origin/main`, then run the **fast checks** — lint, typecheck, build, whatever else runs in seconds without a database or a container — plus, by name, the test files this ticket touched. **Do not run the full gate.** If something fails on code you did not touch, check whether it also fails on a clean `origin/main` before debugging it — a broken base is the repo's problem, not this ticket's to absorb.
